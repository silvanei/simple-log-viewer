...
use S3\Log\Viewer\EventBus\Event\LogCleared;
use S3\Log\Viewer\EventBus\Event\LogReceived;
use S3\Log\Viewer\EventBus\Event\StreamCreated;
use S3\Log\Viewer\EventBus\EventBus;
...
readonly class LogService
{
    ...
    {
        $this->eventBus->dispatch(new StreamCreated($stream, $id));
    }
    ...
    public function add(array $log): void
    {
        ...
        $this->eventBus->dispatch(new LogReceived());
    }
    ...
    {
        $this->storage->clear();
        $this->eventBus->dispatch(new LogCleared());
    }
